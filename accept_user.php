...
if (isset($_POST['email']) && isset($_POST['userName'])) {
    ...
    $sql = "UPDATE guardians SET status='accepted' WHERE email='$email' AND user_name='$userName'";

    if ($conn->query($sql) === TRUE) {
        if ($conn->affected_rows > 0) {
            $response['status'] = "success";
            $response['message'] = "Status updated to accepted for $userName with email $email";
        } else {
